...
@ActionID(
        category = "Tools",
        id = "com.datlt.datlt_plugin.Uppercase"
)
@ActionRegistration(
        displayName = "#CTL_Uppercase"
)
@ActionReferences({
    @ActionReference(path = "Menu/Tools/DatltPlugin", position = 30),
    // Alt + U
    @ActionReference(path = "Shortcuts", name = "A-u") 
})
@Messages("CTL_Uppercase=UPPERCASE")
public final class Uppercase implements ActionListener {

    private final EditCookie context;

    public Uppercase(EditCookie context) {
        ...
        if (activatedNodes.length > 0) {
            ...
            if (cookie != null) {
                ...
                if (selectedText != null && !selectedText.isEmpty()) {
                    ...
                    // 0. Uppercase
                    String wNewText = Common.uppercase(selectedText);
                    ...
                    Common.showNoti(wInfoIcon, "Uppercase Success", selectedText + " -> " + wNewText);
                } else {
